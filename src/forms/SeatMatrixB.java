package forms;
import java.awt.Color;
import javax.swing.*;
public class SeatMatrixB extends JFrame 
{
  JLabel l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13,l14,l15,l16,l17,l18,l19,l20,l21,l22,l23,l24,l25,l26,l27,l28,l29,l30,l31,l32,l33,l34,l35,l36;
  SeatMatrixB(String title)
  {
    super(title);
    l1=new JLabel("B.Tech.");
    l2=new JLabel("Computer Science and Engineering");
    l3=new JLabel("Information Technology");
    l4=new JLabel("Electronics and Communication");
    l5=new JLabel("Electrical and Electronics");
    l6=new JLabel("Mechanical and Automation");
    l7=new JLabel("General");
    l8=new JLabel("80");
    l9=new JLabel("80");
    l10=new JLabel("80");
    l11=new JLabel("40");
    l12=new JLabel("40");
    l13=new JLabel("OBC");
    l14=new JLabel("20");
    l15=new JLabel("20");
    l16=new JLabel("20");
    l17=new JLabel("10");
    l18=new JLabel("10");
    l19=new JLabel("SC");
    l20=new JLabel("10");
    l21=new JLabel("10");
    l22=new JLabel("10");
    l23=new JLabel("05");
    l24=new JLabel("05");
    l25=new JLabel("ST");
    l26=new JLabel("10");
    l27=new JLabel("10");
    l28=new JLabel("10");
    l29=new JLabel("05");
    l30=new JLabel("05");
    l31=new JLabel("TOTAL");
    l32=new JLabel("120");
    l33=new JLabel("120");
    l34=new JLabel("120");
    l35=new JLabel("60");
    l36=new JLabel("60");
    setLayout(null);
    l1.setBounds(5,10,100,30);
    l2.setBounds(5,50,200,30);
    l2.setForeground(Color.blue);
    l3.setBounds(5,90,200, 30);
    l3.setForeground(Color.blue);
    l4.setBounds(5,130,200,30);
    l4.setForeground(Color.blue);
    l5.setBounds(5,170,200,30);
    l5.setForeground(Color.blue);
    l6.setBounds(5,210,200,30);
    l6.setForeground(Color.blue);
    add(l1);add(l2);add(l3);add(l4);add(l5);add(l6);
    l7.setBounds(250,10,100,30);
    l8.setBounds(260,50,100,30);
    l8.setForeground(Color.red);
    l9.setBounds(260,90,100,30);
    l9.setForeground(Color.red);
    l10.setBounds(260,130,100,30);
    l10.setForeground(Color.red);
    l11.setBounds(260,170,100,30);
    l11.setForeground(Color.red);
    l12.setBounds(260,210,100,30);
    l12.setForeground(Color.red);
    add(l7);add(l8);add(l9);add(l10);add(l11);add(l12);
    l13.setBounds(350,10,100,30);
    l14.setBounds(360,50,100,30);
    l14.setForeground(Color.red);
    l15.setBounds(360,90,100,30);
    l15.setForeground(Color.red);
    l16.setBounds(360,130,100,30);
    l16.setForeground(Color.red);
    l17.setBounds(360,170,100,30);
    l17.setForeground(Color.red);
    l18.setBounds(360,210,100,30);
    l18.setForeground(Color.red);
    add(l13);add(l14);add(l15);add(l16);add(l17);add(l18);
    l19.setBounds(450,10,100,30);
    l20.setBounds(450,50,100,30);
    l20.setForeground(Color.red);
    l21.setBounds(450,90,100,30);
    l21.setForeground(Color.red);
    l22.setBounds(450,130,100,30);
    l22.setForeground(Color.red);
    l23.setBounds(450,170,100,30);
    l23.setForeground(Color.red);
    l24.setBounds(450,210,100,30);
    l24.setForeground(Color.red);
    add(l19);add(l20);add(l21);add(l22);add(l23);add(l24);
    l25.setBounds(550,10,100,30);
    l26.setBounds(550,50,100,30);
    l26.setForeground(Color.red);
    l27.setBounds(550,90,100,30);
    l27.setForeground(Color.red);
    l28.setBounds(550,130,100,30);
    l28.setForeground(Color.red);
    l29.setBounds(550,170,100,30);
    l29.setForeground(Color.red);
    l30.setBounds(550,210,100,30);
    l30.setForeground(Color.red);
    add(l25);add(l26);add(l27);add(l28);add(l29);add(l30);  
    l31.setBounds(640,10,100,30);
    l32.setBounds(645,50,100,30);
    l32.setForeground(Color.red);
    l33.setBounds(645,90,100,30);
    l33.setForeground(Color.red);
    l34.setBounds(645,130,100,30);
    l34.setForeground(Color.red);
    l35.setBounds(650,170,100,30);
    l35.setForeground(Color.red);
    l36.setBounds(650,210,100,30);
    l36.setForeground(Color.red);
    add(l31);add(l32);add(l33);add(l34);add(l35);add(l36);
  }
   public static void main(String[] args)
    {
      SeatMatrixB obj=new SeatMatrixB("Seat Matrix BTech");  
        obj.setSize(710,300);
        obj.setLocation(250,150);
        obj.setDefaultCloseOperation(EXIT_ON_CLOSE);
        obj.setVisible(true);
    }
}
